...
const InstancePreviewModal: React.FC<InstancePreviewModalProps> = ({
    ...
    useEffect(() => {
        ...
        .catch((error: any) => {
            console.error('Error preview instance: ', error);
            showSnackbar({
                isLowContrast: false,
                kind: 'error',
                title: t('errorPreviewInstance', 'Error while getting the preview of the instance'),
    ...
    return (
        <div className='instancePreviewDic' style={{textAlign: 'center'}}>
            <ModalHeader closeModal={closeInstancePreviewModal} title={t('instancePreview', 'Preview instance')} />
            <ModalBody>
